    ...
    def create_chart(self, df: pd.DataFrame, signal: Dict, coin: str, timeframe: str = '15m') -> BytesIO:
        ...
        if 'fvg_zones' in signal and signal['fvg_zones']:
            ...
            for zone in fvg_to_show:
                ...
                tf_display = zone.get('tf', '?')
                label = f"FVG {tf_display} ({zone.get('size', 0):.1f}%)"
...
class MultiTimeframeAnalyzer:
    ...
    def generate_signal(self, dataframes: Dict[str, pd.DataFrame], metadata: Dict, symbol: str, exchange: str) -> Optional[Dict]:
        ...
        # # ===== РАСЧЕТ ГАРАНТИРОВАННЫХ ЗОН ДОБОРА =====
        ...
        #             if abs(price - seen) / price < 0.01:  # 1% допуск
        #                 is_dup = True
        #                 break
        #         if not is_dup:
        ...
        # entry_zones = unique_zones[:3]  # Максимум 3 зоны
...
class FastPumpScanner:
    ...
    def format_pump_message(self, signal: Dict, contract_info: Dict = None, dataframes: Dict = None) -> Tuple[str, InlineKeyboardMarkup]:
        """
...
class MultiExchangeScannerBot:
    ...
    async def send_signal(self, signal: Dict, pump_only: bool = False):
        ...
        #     # Зоны посчитаны для LONG — очищаем
        #     signal['entry_zones'] = []
        
        msg, keyboard = self.format_message(signal, contract_info, pump_percent, dataframes=dataframes)
...
class TelegramHandler:
    ...
    async def groups_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            "*🔹 ОСНОВНАЯ ГРУППА*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Обычные LONG/SHORT сигналы на основе технического анализа.\n"
            "• RSI, MACD, EMA 21/50/200, VWAP\n"
            "• Фибоначчи, дивергенции, фракталы\n"
            "• Уровни поддержки/сопротивления\n"
            "• Мультитаймфреймовое подтверждение\n"
            "• Графики с индикаторами\n"
            "• Зоны добора (исторические мин/макс 15м)\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*🚨 ПАМП-ГРУППА*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Резкие движения цены — PUMP (рост) и DUMP (падение).\n"
            "• Мейджоры: от 5% движения\n"
            "• Щиткоины: от 7% движения\n"
            "• Сканирование каждые 30 секунд\n"
            "• LONG на отскоке после дампа\n"
            "• SHORT на коррекции после пампа\n"
            "• Цели, стоп-лосс, зоны добора\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📦 НАКОПЛЕНИЕ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Ранние сигналы ДО начала импульсного движения.\n"
            "• Аномальный рост объёма при стабильной цене\n"
            "• Дивергенции RSI/MACD (разворотные)\n"
            "• Имбалансы (FVG) — зоны незаполненного объёма\n"
            "• Сжатие волатильности (Bollinger Squeeze)\n"
            "• Позволяет войти раньше основного движения\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*💎 VIP СИГНАЛЫ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Усиленные сигналы с высокой вероятностью.\n"
            "• Совпадение нескольких индикаторов\n"
            "• Подтверждение на нескольких таймфреймах\n"
            "• Повышенное соотношение риск/прибыль\n"
            "• Ликвидность выше среднего\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📈 СТАТИСТИКА*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Отчёты и метрики по работе бота.\n"
            "• /stats — общая за 7 дней\n"
            "• /stats\\_pump — памп-дамп\n"
            "• /stats\\_vip — VIP сигналы\n"
            "• /stats\\_regular — обычные\n"
            "• /stats\\_accumulation — накопление\n"
            "• Кнопки: по монетам, помощь\n",
            parse_mode='Markdown'
        )
    
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text(
            "*📚 ПОМОЩЬ — Мульти-биржевой сканер*\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📡 КАК РАБОТАЕТ БОТ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Бот сканирует 597 пар на BingX Futures каждые 30 секунд "
            "и отправляет сигналы при обнаружении сильных движений.\n\n"

            "*Типы сигналов:*\n"
            "🚨 *PUMP/DUMP* — резкий рост/падение цены (>3-7%)\n"
            "📊 *Обычные* — LONG/SHORT на основе теханализа\n"
            "📦 *Накопление* — ранние сигналы до импульса\n"
            "💎 *VIP* — усиленные сигналы с высокой вероятностью\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📊 ТЕХНИЧЕСКИЙ АНАЛИЗ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "• *RSI* — индекс относительной силы\n"
            "• *MACD* — схождение/расхождение скользящих\n"
            "• *EMA 21/50/200* — экспоненциальные средние\n"
            "• *VWAP* — средневзвешенная цена по объёму\n"
            "• *Фибоначчи* — коррекции и расширения\n"
            "• *Дивергенции* — расхождение цены и индикаторов\n"
            "• *Имбалансы (FVG)* — зоны незаполненного объёма\n"
            "• *Фракталы* — разворотные паттерны\n"
            "• *EQH/EQL* — равные максимумы/минимумы (ликвидность)\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📖 ЧТЕНИЕ СИГНАЛА*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📊 *Направление* — LONG (покупка) или SHORT (продажа)\n"
            "🕓 *Таймфрейм* — на каком ТФ обнаружен сигнал\n"
            "💰 *Цена* — текущая цена входа\n"
            "🎯 *Цели* — уровни тейк-профита и стоп-лосса\n"
            "🟣 *Зоны добора* — уровни для усреднения позиции "
            "(рассчитаны по историческим мин/макс на 15м)\n"
            "📊 *Риск/Прибыль* — соотношение риска к потенциалу\n"
            "💡 *Причины* — почему сгенерирован сигнал\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*🔘 КНОПКИ ПОД СИГНАЛОМ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "📋 *Копировать* — показать название монеты\n"
            "🔄 *Обновить* — обновить сигнал актуальными данными\n"
            "📊 *Детали* — развёрнутый технический анализ\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📋 КОМАНДЫ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "/start — запуск бота\n"
            "/stats — общая статистика за 7 дней\n"
            "/stats\\_pump — статистика памп-дамп сигналов\n"
            "/stats\\_vip — статистика VIP сигналов\n"
            "/stats\\_regular — статистика обычных сигналов\n"
            "/stats\\_accumulation — статистика накопления\n"
            "/status — статус и состояние бота\n"
            "/help — эта справка\n\n"

            "━━━━━━━━━━━━━━━━━━━━\n"
            "*📊 ГРУППЫ*\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "🔹 *Основная* — обычные LONG/SHORT сигналы\n"
            "🔹 *Памп-группа* — PUMP/DUMP движения >3%\n"
            "🔹 *Накопление* — ранние сигналы до импульса\n"
            "🔹 *VIP* — усиленные сигналы\n"
            "🔹 *Статистика* — отчёты и метрики\n",
            parse_mode='Markdown'
        )
        ...
        if data.startswith("refresh_"):
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"⏰ Таймаут графика {coin}, отправляю без графика")
                    await context.bot.send_message(
            ...
            try:
                ...
                logger.error(f"❌ Ошибка деталей {coin}: {e}")
                await query.answer(f"❌ Ошибка: {e}", show_alert=True)
            return
